...
import requests
from scraper import BASE_URL, MUSIC_DIR
...
def get_anthem_names(soup):
    ...
    anthem_names = [
        line
        for line in anthem_title_section.find(
            "div", class_="customvalue"
        ).stripped_strings
    ]
...
def get_composers(soup):
    field_ids = ["custom-field-4", "custom-field-10"]
    composers = []

    for field_id in field_ids:
        div = soup.find("div", {"id": field_id})
        if div:
            composer_div = div.find("div", class_="customvalue")
            if composer_div:
                composers.extend([line.strip() for line in composer_div.stripped_strings])
...
def download_music(country, soup):
    ...
    return music_file_path.as_posix() 
